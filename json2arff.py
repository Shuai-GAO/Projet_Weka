...
def get_stats_longueur(liste):
  ...
  return {"L_moyenne_mots":get_moyenne(longueurs, 2), "L_min_mots": min(longueurs), "L_max_mots":max(longueurs)} # Transformer les fichiers dans Brown en une liste des longueurs des mots
...
def get_types_phrases(chaine): # Il faut modifier
  nb_affirm = len(re.findall("\. ", chaine)) # \ déspécialiser
...
def get_stats_occurrence(liste, json):
	occu_present_tense = 0
	occu_past_tense = 0
	usual_verbs_set = lire_json(json)
	for verb in liste:
		if verb in usual_verbs_set["verbs_present_tense"]:
			occu_present_tense += 1
		if verb in usual_verbs_set["verbs_past_tense"]:
			occu_past_tense += 1   
	return {"verbs_present_tense": occu_present_tense, "verbs_past_tense": occu_past_tense}